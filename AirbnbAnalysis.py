...
def datafr():
    df=pd.read_csv("C:/Users/Praveena/OneDrive/Desktop/projects/Airbnb/Airbnb_Analysis.csv")
    return df
df=datafr()
...
if select=="Home":
    image1=Image.open("C:/Users/Praveena/OneDrive/Desktop/projects/Airbnb/Airbnb logo.png")
    st.image(image1)

    st.header("About Airbnb")
    st.write("")
    st.write('''Airbnb (ABNB) is an online marketplace that connects
              people who want to rent out their homes with people looking
              for accommodations in specific locales. The company has
              come a long way since 2007, when its co-founders first
              came up with the idea to invite paying guests to sleep 
             on an air mattress in their living room. According to 
             Airbnb's latest data, it now has more than 7 million 
             listings, covering some 100,000 cities and towns in 
             220-plus countries and regions worldwide.''')
    st.write("")
    st.write('''***Airbnb has revolutionized the hospitality industry. 
             Prior to 2008, when the company officially launched, 
             travelers would typically book a hotel or motel room for 
             their trip to another town. Nowadays, many of these 
             same people are opting for Airbnb.***''')
    st.header("Impact on Airbnb")
    st.write("")
    st.write('''***On Travel*** Airbnb has made travel more feasible 
             for many people by providing a way to find accommodations 
             that cost less than traditional hotels and other lodging. 
             It has provided property owners in destination cities with 
             new opportunities to earn income by renting out available 
             space to these travelers.''')
    st.write('''***On Its Local Markets*** The profusion of Airbnb 
             rentals has brought tourism income to shops and restaurants 
             in some previously off-the-beaten path neighborhoods. But 
             critics contend that it can also bring the downside of 
             tourism, such as more noise and rowdy behavior.''')
...
if select=="Data Exploration":
    ...
    with tab2:
        ...
        def datafr():
            df_a=pd.read_csv("C:/Users/Praveena/OneDrive/Desktop/projects/Airbnb/Airbnb_Analysis.csv")
            return df_a
    ...
    with tab3:
        ...
        def datafr():
            df_l=pd.read_csv("C:/Users/Praveena/OneDrive/Desktop/projects/Airbnb/Airbnb_Analysis.csv")
            return df_l
...
if select == "About":
    ...
    st.write("This project is done by Prashanthi.")